...
def detect_date_columns(df):
    """
    Detect genuine date columns in a DataFrame.
    Delegates to the shared implementation in datetime_utils.
    """
    return shared_detect_dates(df)

def detect_data_types(df):
    # Use the same dtype-based detection as preprocessing_service._get_column_info()
    numeric = list(df.select_dtypes(include=[np.number]).columns)
    categorical = list(df.select_dtypes(include=['object', 'category']).columns)

    # Remove date columns from categorical — dates are not categorical
    date_cols = shared_detect_dates(df)
    categorical = [c for c in categorical if c not in date_cols]
