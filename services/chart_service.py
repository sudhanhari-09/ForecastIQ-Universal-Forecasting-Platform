...
def sanitize_filename(name):
    """Replace filesystem-invalid characters and spaces with underscores."""
    invalid_chars = '/\\:*?"<>|'
    result = str(name)
...
def histogram(df, column, output_dir):
    ...
    img_path = os.path.join(output_dir, 'images', f'histogram_{sanitize_filename(column)}.png')
...
def density_plot(df, column, output_dir):
    ...
    try:
        ...
        img_path = os.path.join(output_dir, 'images', f'density_{sanitize_filename(column)}.png')
        _save_matplotlib(mpl_fig, img_path)
        return _plotly_to_json(fig), img_path
    except Exception as e:
...
def boxplot(df, column, output_dir):
    ...
    img_path = os.path.join(output_dir, 'images', f'boxplot_{sanitize_filename(column)}.png')
    _save_matplotlib(mpl_fig, img_path)
    return _plotly_to_json(fig), img_path
...
def line_chart(df, x_col, y_col, output_dir, title=None):
    ...
    img_path = os.path.join(output_dir, 'images', f'line_{sanitize_filename(y_col)}.png')
    _save_matplotlib(mpl_fig, img_path)
    return _plotly_to_json(fig), img_path
...
def bar_chart(freq_dict, title, output_dir, filename='bar_chart'):
    ...
    img_path = os.path.join(output_dir, 'images', f'{sanitize_filename(filename)}.png')
    _save_matplotlib(mpl_fig, img_path)
    return _plotly_to_json(fig), img_path
...
def trend_chart(df, date_col, value_col, output_dir):
    ...
    img_path = os.path.join(output_dir, 'images', f'trend_{sanitize_filename(value_col)}.png')
    _save_matplotlib(mpl_fig, img_path)
    return _plotly_to_json(fig), img_path
...
def rolling_average_chart(df, date_col, value_col, window, output_dir):
    ...
    ax.legend()
    img_path = os.path.join(output_dir, 'images', f'rolling_{sanitize_filename(value_col)}.png')
    _save_matplotlib(mpl_fig, img_path)
    return _plotly_to_json(fig), img_path
