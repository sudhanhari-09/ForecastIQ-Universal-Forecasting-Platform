...
def _log_error(chart_name, column, error):
    traceback.print_exc()
    current_app.logger.error("EDA error - %s | Column: %s | %s", chart_name, column, error)
...
def analyze_timeseries(df, date_col=None):
    ...
    # Auto detect datetime column using shared implementation
    if date_col is None:
...
def _detect_datetime_columns(df):
    """
    Detect datetime columns including object/string columns.
    Delegates to the shared implementation in datetime_utils.
    """
...
<style>
  ...
  .badge {{ display: inline-block; padding: 3px 8px; border-radius: 4px; font-size: 12px; color: #212529; background: #e9ecef; }}
