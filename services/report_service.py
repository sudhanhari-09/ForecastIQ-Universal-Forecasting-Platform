...
def _generate_smart_insights(ctx):
    ...
    if prep:
        ...
        missing_method = (steps.get('missing_values') or {}).get('method')
        if missing_method and missing_method != 'none':
            insights.append(f'Missing values handled using {missing_method.replace("_", " ")} method.')
        outlier_method = (steps.get('outliers') or {}).get('method')
        if outlier_method and outlier_method != 'none':
            insights.append(f'Outliers treated with {outlier_method.replace("_", " ")} method.')
...
def generate_full_report(dataset_id):
    ...
    if comp and comp.best_model != 'N/A':
        best_model_name = comp.best_model
    if not best_model_name and comp_results:
        best_model_name = comp_results.get('best_model')
    if not best_model_name and fc:
        best_model_name = fc.model_name
    best_explanation = ''
    ranking = []
    insights = []
    chart_data = {}
    ...
    if prep:
        ...
        preprocessing_data = {
            ...
            'missing_method': (steps or {}).get('missing_values', {}).get('method', 'N/A'),
            'outlier_method': (steps or {}).get('outliers', {}).get('method', 'N/A'),
            'encoding_method': (steps or {}).get('encoding', {}).get('method', 'N/A'),
            'scaling_method': (steps or {}).get('scaling', {}).get('method', 'N/A'),
        }

    forecast_models = {}
    forecasts = []
    target_col = forecast_results.get('target_column', 'Value') if forecast_results else 'Value'
    date_col = forecast_results.get('date_column', 'Date') if forecast_results else 'Date'
    if forecast_results:
    ...
    report = Report.query.filter_by(dataset_id=dataset_id).order_by(
        Report.created_at.desc()
    ).first()
    if not report:
